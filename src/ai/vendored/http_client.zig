...
//!
//! VENDORED COPY of std/http/Client.zig from Zig 0.17.0-dev.87+9b177a7d2 with
//! Zig PR #23365 applied: when CONNECT-tunnelling to an https origin through
//! a forward proxy, perform a TLS handshake on the established tunnel before
//! sending the request body. Without this patch, requests to HTTPS hosts via
//! HTTP_PROXY/HTTPS_PROXY are sent as plaintext through the CONNECT tunnel,
//! which causes Squid-style proxies to reject them ("Host header does not
...
// v1.26.6 — diagnostic logging for the CONNECT-tunnel swap path.
// One real-user crash (Linux ARM64, kernel-space fault address
// `0xffff7bb80050` during `sendHead`) couldn't be diagnosed from
// the trace alone. Until we have a repro, every step of the swap
// emits a TRACE log line so the next crash can be correlated with
// state — opt in via `--log-level trace` / `FRANKY_LOG=trace`,
...
///
/// `tunnel_protocol` is the protocol of the origin server reached through the
/// tunnel. When `.tls`, a TLS handshake is performed against the origin after
/// the CONNECT response so that subsequent requests are encrypted end-to-end.
/// (Patched per Zig PR #23365 — without this upgrade the request body is sent
/// as plaintext through the CONNECT tunnel and Squid-style proxies reject it
/// with "Host header does not match CONNECT request".)
///
/// This function is threadsafe.
pub fn connectProxied(
    ...
    // v1.26.6 — entry log. Captures destination + intent so a later
    // crash trace can be correlated with which call into the swap
    // path is in flight. `proto={s}` is `tls` or `plain`.
    tunnel_log.log(.trace, "http", "tunnel.enter", "host={s} port={d} proto={s}", .{
        proxied_host.bytes, proxied_port, @tagName(tunnel_protocol),
    });
    ...
    if (try client.connection_pool.findConnection(io, .{
        ...
        tunnel_log.log(.trace, "http", "tunnel.pool_hit", "host={s} port={d} conn=0x{x}", .{
            proxied_host.bytes, proxied_port, @intFromPtr(node),
        });
        return node;
    }
    ...
    (tunnel: {
        ...
        // After connectTcpOptions: we have a fresh Plain wrapper around
        // the TCP socket to the proxy. Logging the connection pointer
        // lets us trace the same wrapper through the swap.
        tunnel_log.log(.trace, "http", "tunnel.proxy_connected", "proxy_host={s} proxy_port={d} conn=0x{x}", .{
            proxy.host.bytes, proxy.port, @intFromPtr(connection),
        });
        var owns_plain = true;
        errdefer if (owns_plain) {
            connection.closing = true;
        ...
        var req = client.request(.CONNECT, .{
            ...
            tunnel_log.log(.warn, "http", "tunnel.request_init_failed", "err={s}", .{@errorName(err)});
            break :tunnel err;
        };
        defer req.deinit();

        req.sendBodiless() catch |err| {
            tunnel_log.log(.warn, "http", "tunnel.send_connect_failed", "err={s}", .{@errorName(err)});
            break :tunnel err;
        };
        const response = req.receiveHead(&.{}) catch |err| {
            tunnel_log.log(.warn, "http", "tunnel.recv_connect_failed", "err={s}", .{@errorName(err)});
            break :tunnel err;
        };
        tunnel_log.log(.trace, "http", "tunnel.connect_response", "status={d}", .{
        ...
        if (tunnel_protocol == .tls) {
            if (disable_tls) break :tunnel error.TlsInitializationFailed;

            // Take ownership of the underlying TCP stream from the Plain
            // wrapper and rebuild a TLS-wrapped Connection around it so the
            // request body is sent through a real TLS handshake against the
            // origin (not in plaintext through the CONNECT tunnel).
            //
            // v1.26.6 — every step of the swap is logged so a post-mortem
            // can reconstruct exactly where state went bad. Pointer values
            // double as fingerprints: if a subsequent crash address shows
            // up in earlier logs, that's the smoking gun for a UAF.
            const tcp_stream = connection.stream_reader.stream;
            tunnel_log.log(.trace, "http", "tunnel.swap_begin", "host={s} plain_conn=0x{x}", .{
                proxied_host.bytes, @intFromPtr(connection),
            });
            ...
            tunnel_log.log(.trace, "http", "tunnel.plain_detached", "conn=0x{x}", .{@intFromPtr(connection)});
            const plain: *Connection.Plain = @alignCast(@fieldParentPtr("connection", connection));
            const plain_addr = @intFromPtr(plain);
            plain.destroy();
            owns_plain = false;
            ...
            const tls = Connection.Tls.create(client, proxied_host, proxied_port, tcp_stream) catch |err| switch (err) {
                ...
                error.Unexpected => |e| {
                    tunnel_log.log(.warn, "http", "tunnel.tls_create_unexpected", "host={s}", .{proxied_host.bytes});
                    tcp_stream.close(io);
                    return e;
                },
                error.Canceled => |e| {
                    tunnel_log.log(.warn, "http", "tunnel.tls_create_canceled", "host={s}", .{proxied_host.bytes});
                    tcp_stream.close(io);
                    return e;
                },
                else => {
                    tunnel_log.log(.warn, "http", "tunnel.tls_create_failed", "host={s} err={s}", .{ proxied_host.bytes, @errorName(err) });
                    tcp_stream.close(io);
                    break :tunnel error.TlsInitializationFailed;
                },
            };
            // After Tls.create: log the new wrapper's identifying
            // pointers. `tls` is the heap-allocated Tls struct;
            // `tls.connection` is the embedded Connection pushed to
            // the pool; `tls.client.writer` is what `connection.writer()`
            // returns to `sendHead`. If a later crash address matches
            // `tls_writer_buf` ± a small offset, we know the writer's
            // buffer was the corrupt slice.
            const writer_iface: *const std.Io.Writer = &tls.client.writer;
            tunnel_log.log(.trace, "http", "tunnel.tls_created", "tls=0x{x} conn=0x{x} writer=0x{x} writer_buf_ptr=0x{x} writer_buf_len={d}", .{
                @intFromPtr(tls),
                @intFromPtr(&tls.connection),
                @intFromPtr(writer_iface),
                @intFromPtr(writer_iface.buffer.ptr),
            ...
            tunnel_log.log(.trace, "http", "tunnel.tls_pooled", "conn=0x{x}", .{@intFromPtr(&tls.connection)});
            return &tls.connection;
        }

        tunnel_log.log(.trace, "http", "tunnel.plain_returned", "conn=0x{x}", .{@intFromPtr(connection)});
        return connection;
    }) catch {
