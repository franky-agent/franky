...
> **Status: concluded at v1.31.0 (2026-05-01). v2.0.0 cut on the same day.**
> The v1.x line is closed — this file is now a read-only historical reference
> for the architecture and shipped behavior of the v1.0.0 → v1.31.0 binaries.
> All new roadmap work goes to [`v2.md`](v2.md) (v2.x active spec) or
> [`v3.md`](v3.md) (next-major); per-version history goes to [`CHANGELOG.md`](../../CHANGELOG.md).
> Edits to this file are limited to (a) fixing factual errors and (b) flipping
> rows to `❌` if a v1.x-introduced feature is later removed under a v2.x cut.
...
## Implementation status — v1.x line (v1.0.0 → v1.31.0, concluded)

Status of each section against the code in `franky/src/` over the
v1.x line (v1.0.0 cut 2026-04-24, line concluded at v1.31.0 on
2026-05-01). Legend:
