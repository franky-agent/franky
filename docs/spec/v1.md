...
OAuth user-facing (later removed in v1.30.0 — see that row in the log
below), HTTP hardening, agent-loop depth, persistence tree, web UI,
capability roles, permission overlay, logging refinements, per-phase
HTTP timeouts.
...
row in the implementation-status table above showed ✅ at the v1.0.0
cut. The OAuth-related rows (§3.8 minting half, §Q minting flows)
flipped to `❌ removed in v1.30.0` after the in-tree minting tree
was deleted; see the v1.30.0 row in the "what shipped" log. Items
that did not ship at this cut live in **v2.md**.
...
| v1.2.* | OAuth orchestrator (removed v1.30.0) | §Q.1/Q.2/Q.3/Q.5, anthropic/copilot/gemini/vertex login — entire `src/coding/oauth/` tree + `franky login` subcommand removed in v1.30.0 |
...
| v1.30.0 | **Remove in-tree OAuth minting (`src/coding/oauth/` + `franky login`)** | The PKCE listener, device-code poller, and RS256 JWT signer that v1.2.\* introduced are gone — `src/coding/oauth/` (13 files, ~2,900 LOC) and `src/coding/modes/login.zig` (~355 LOC) deleted; `franky login` dispatch dropped from `src/bin/main.zig`; `pub const oauth` and `pub const modes.login` removed from `src/coding/mod.zig`; `/login` and `/logout` lines removed from `src/coding/slash.zig`'s help block and the matching line in interactive's help overlay. **Kept (so this is a removal of *minting*, not of bearer auth):** `--auth-token` CLI flag and `ANTHROPIC_AUTH_TOKEN` / `CLAUDE_CODE_OAUTH_TOKEN` env-var fallbacks; `auth_token` / `auth_token_env` profile fields; `Options.auth_token` in `ai/registry.zig`; `ProviderAuthType.oauth` enum variant in `coding/auth.zig` (so `auth.json` still round-trips OAuth-shaped records); the bearer-token paths in `ai/providers/anthropic.zig` (`Authorization: Bearer <token>` + `anthropic-beta: claude-code-20250219,oauth-2025-04-20` + Claude Code system-prompt prefix per §A.2.1) and `ai/providers/google_gemini.zig` (Google bearer path). New flow: users mint long-lived tokens externally (e.g. `claude setup-token` from the official Claude Code CLI for Anthropic Pro/Max/Team/Enterprise; vendor-specific tooling for other providers) and either pass via `--auth-token`/env var or hand-write a `type: "oauth"` record into `auth.json`. Spec rows updated: §3.8 + §Q (rows in this table show `❌ minting removed in v1.30.0`); §J slash-commands row notes the dropped `/login`/`/logout` entries; the "Auth state" paragraph rewritten. **Tests:** 978 → 908 (−70, all OAuth/login coverage gone with the code). The §Q.1–§Q.6 sections retained verbatim as a historical reference for the v1.2.\*–v1.29.\* implementation; §Q.5 (resolver) and §Q.7 (consuming pre-minted tokens) still describe live behavior. |
| v1.29.7 | **Fix proxy-arena leak in `setupClientFromEnv` without diverging further from upstream `Client`** | The v1.29.4 trade-off (each Slack/LLM HTTP call leaks ~100B for the lifetime of the process — Proxy struct + duped host + optional Basic-auth blob) became visible in DebugAllocator runs of franky-do as ~26 leak reports per session, all routing through `setupClientFromEnv`. The root cause was the v1.28.x bug-and-fix sequence: v1.28.x scoped a function-local `proxy_arena` to `setupClientFromEnv`, which freed the proxies BEFORE the client died (vendored `Client` field doc: "Uses `arena` for a few small allocations that must outlive the client"). v1.29.4 routed around the UAF by passing the caller's long-lived allocator straight in — fixing the segfault but never freeing. **v1.29.7 honors the upstream contract literally**: `setupClientFromEnv` now creates an `ArenaAllocator` internally, hands its allocator to `initDefaultProxies`, and **returns the arena to the caller**. Caller pairs the arena's deinit with the client's via a single `defer { client.deinit(); proxy_arena.deinit(); }` block — the client tears down first (using proxy pointers that are still alive), then the arena frees them. Single defer block is intentional: two separate defers would LIFO-order against intent. **No vendored-Client changes** (the `vendored/http_client.zig` patches stay at v1.26.0's PR-#23365 baseline); this is purely a wrapper-side refactor of franky's existing `setupClientFromEnv` helper. **Breaking signature change**: `setupClientFromEnv` returns `!std.heap.ArenaAllocator` instead of `!void`. Five franky provider call sites (anthropic, google_gemini, google_vertex, openai_chat, openai_responses) updated; franky-do's two Slack-API call sites update in lockstep (v0.5.5). **+2 tests** (`setupClientFromEnv: returns ArenaAllocator that frees Proxy on deinit (no leak)` — sets `https_proxy` env, runs setup, asserts both proxy fields populated and port parsed, exercises the testing.allocator's leak detector at scope exit; `setupClientFromEnv: empty environ → empty arena that still deinits cleanly` — asserts the no-proxies path still produces a deinit-able arena). **978 tests passing total** (was 976; +2). |
| v1.29.6 | **`/diagnostics`: provider/model in header + sharper edit/write hints** | Real-user franky-do incident (gemini-2.5-pro, 54-turn session): 17 of 17 anomalies were `TOOL_ERROR`, dominated by `edit_no_match` (10×). The model interpreted the existing hint ("Read the file again to get the current bytes") as "widen `old` with more surrounding context," then re-failed on the bigger guess, then widened again, etc. The pre-fix hints also routed `edit_ambiguous`, `write_exists`, and `invalid_args` to the generic fallback even though each has a clear distinct recovery. **Two pieces.** (1) `Options.provider` + `Options.model` (display-only `?[]const u8` fields) added to `coding/diagnostics.zig`; rendered in the header as `provider:    google` / `model:       gemini-2.5-pro` (defaults to `?` when unset). franky-do consumers pass these from `bot.Config.model_provider` / `bot.Config.model_id`. (2) `hintForToolError` split + extended: **`edit_no_match`** now reads `Edit \`old\` not found. DO NOT widen \`old\` with more context — that won't help if the original guess is wrong. Re-read the file with the \`read\` tool to get the actual bytes, then copy-paste the exact text into \`old\`.` (anti-widening framing matters because Gemini was reading "read again" as "widen"); **`edit_ambiguous`** is the OPPOSITE recovery — `Widen \`old\` with more surrounding context (e.g. include the line above and below) until it uniquely identifies the target. If a function-/declaration-level match is still ambiguous, split into two edits.`; **`write_exists`** points at `overwrite: true` AND suggests using `edit` instead of rewriting; **`invalid_args`** points at the message body (which always names the bad/missing field). Legacy "no match" message-body fallback rewritten with the same anti-widening framing. **+7 tests** (`edit_no_match steers AWAY from widening`, `edit_ambiguous DOES tell the model to widen`, `write_exists points at overwrite=true and edit alternative`, `invalid_args points at the error message`, `legacy no-match-in-message fallback still fires`, `header includes provider + model when set`, `header shows '?' when provider/model unset`). **976 tests passing total** (was 969; +7). The pre-existing `edit_no_match` row in `hintForToolError: recognizes canonical codes` was removed since the assertion (`Read the file again`) was the literal text we just deleted; the four new dedicated tests above cover that code now. |
| v1.29.5 | **Hard-skip `.git/` in `ls` and `find` tools** | Real-user franky-do report: an `ls` recursive run from repo root flooded the LLM context with thousands of `.git/objects/<sha>/` entries. Root cause: the v1.26.2 `.git/` hard-skip went into `grep` only — `ls` and `find` were left relying on `respectGitignore`, but `.git/` is git's internal data store, not a tracked-or-ignored path, so it never appears in any `.gitignore` and the gitignore filter was a no-op against it. **Fix** mirrors `grep.zig:271-275` exactly: in `ls.zig`'s recursive arm `if (std.mem.eql(u8, entry.path, ".git") or std.mem.startsWith(u8, entry.path, ".git/")) continue;` before the gitignore stack check; in the non-recursive arm `if (std.mem.eql(u8, entry.name, ".git")) continue;` (single-component name comparison). `find.zig` gets the same recursive-form guard. **+2 tests** (`ls tool: hard-skips .git directory regardless of respectGitignore` — sets up `.git/{config,objects/ab/cdef}` plus a real source file, asserts both recursive and non-recursive listings see the source file but no `.git`, `config`, `objects`, or `abcdef`; `find tool: hard-skips .git directory regardless of respectGitignore` — analogous setup, asserts a `**/*` glob from the test root returns the source file but neither `.git/config` nor `.git/objects/ab/cdef`). Both tests deliberately set `respect_gitignore=false` to prove the skip is independent of gitignore. **969 tests passing total** (was 967; +2). |
| v1.29.4 | **Fix proxy use-after-free in `setupClientFromEnv` (segfault on `https_proxy.host.eql`)** | Real-user franky-do crash with full stack trace landing on `vendored/http_client.zig:1737:23 in connect`: `if (proxy.host.eql(host) and proxy.port == port and proxy.protocol == protocol)` — `proxy` was a freed pointer. Fault address `0xffff936e0040` matched the v1.26.x high-bit user-space UAF pattern the v1.28.1 spec row already framed correctly. Reproduction: any operator with `HTTPS_PROXY` set (Docker Sandboxes' Squid, corporate MITM, etc.) hitting franky's HTTP layer through `setupClientFromEnv`. The franky-do session ran fine on the first request (proxy memory not yet recycled by the GeneralPurposeAllocator), then crashed on a subsequent request once the freed page got reused. **Root cause:** `setupClientFromEnv` (lines 92–94 pre-fix) created a function-scoped `var proxy_arena = std.heap.ArenaAllocator.init(allocator); defer proxy_arena.deinit();` and passed `proxy_arena.allocator()` to `client.initDefaultProxies(...)`. The vendored Zig Client's documented contract (`vendored/http_client.zig:1331-1334`) is that `http_proxy`/`https_proxy` are "pointers to externally-owned memory" that **must outlive the client**. franky's wrapper violated that: the arena died on `setupClientFromEnv` return, but the client's two proxy fields kept pointing into the freed memory until the next request's `connect()` deref'd them. **Fix** (one-line — in `src/ai/http.zig`'s `setupClientFromEnv`): drop the function-scoped arena and pass the caller's `allocator` (long-lived, the same one the client uses for its CA bundle and connection pool) directly into `client.initDefaultProxies`. Trade-off: each call leaks ~100 bytes (Proxy struct + host string + optional Basic-auth blob) for the lifetime of the process. For typical workloads — single-process bots making a few hundred HTTP calls per day — that's ~tens of KB/day, well under bot-restart cadences. The proper fix (a Proxy destructor on the vendored Client) is deferred to v2.x because it requires diverging further from upstream Zig stdlib than the v1.26.0 PR-#23365 patch already does. **No new tests** (the bug only manifests under a real HTTPS_PROXY in production; the test allocator's per-test reset masks the UAF). All 967 existing tests pass clean. **Spec note in `docs/spec/v2.md` §F** for the deferred destructor work. |
...
> **Status (v1.30.0).** The in-tree minting flows described in §Q.1–§Q.4
> were removed in v1.30.0 along with `src/coding/oauth/` and the
> `franky login` subcommand. Sections §Q.1–§Q.4 and §Q.6 are retained
> verbatim as a historical reference for the v1.2.\*–v1.29.\*
> implementation. **Live behavior** lives in §Q.5 (auth resolver) and
> §Q.7 (consuming pre-minted bearer tokens — the "paste a long-lived
> token from `claude setup-token` into `--auth-token`" path). See the
> v1.30.0 row in the "what shipped" log for the rationale and what
> persisted (`Options.auth_token`, `auth.json` round-trip,
> `ProviderAuthType.oauth` enum, the Anthropic / Gemini bearer paths).
