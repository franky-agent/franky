...
- Extend `src/coding/config/settings.zig` to parse `profiles` map +
  `${VAR}` interpolation + `api_key_env` indirection + `env`
  block.
- Add `--profile <name>` flag in `src/coding/config/cli.zig`.
...
- **v2.2.0 — `franky replay` + provider fixture suite (closes
  §4.14 #2 + #3).** New `src/coding/session/replay.zig` (~440 LOC):
...
- **v2.3.0 — `franky fixture` subcommand + diagnostics-to-fixture
  ...
  `pub fn redactTraceText` in `src/coding/session/replay.zig` is the
...
- **v2.9.0 — `.contextignore` (archived/historical paths hidden from
  ...
  **+9 tests; 1032 → 1041.** Files touched: `coding/gitignore.zig`,
  `coding/security/path_safety.zig`, `coding/tools/common.zig`,
...
### 6.11 Self-restart via spawn-and-exit (v2.17)

Shipped. Self-restart mechanism that closes the listen socket, spawns a fresh
instance of the running binary, and exits the current process. Three triggers:

- **`/restart` slash command** (interactive mode) — sets `restart_requested`
  flag; the main loop polls the flag between turns, then calls
  `restart.spawnAndExit(io)`.
- **`POST /restart` HTTP route** (proxy mode) — same flag-based mechanism;
  closes the listen socket before spawning so the child can bind the same
  port.
- **`finish_task` with `restart: true`** (model-driven) — the guardrail sets
  the session's `restart_requested` flag via `GuardrailState.restart_requested`;
  picked up by both interactive and proxy mode loops.

Implementation:
- `src/coding/restart.zig` — `init(argv, io, allocator)` caches the exe path
  + argv; `spawnAndExit(io)` resolves exe path, spawns child (inherit stdio),
  sleeps 200 ms for bind handoff, then `std.process.exit(0)`.
- `src/coding/modes/interactive.zig` — `restart_requested` atomic flag on the
  Session struct; polled in the main loop between frames and after the loop
  exits (`restart_mod.spawnAndExit`). `/restart` slash command sets the flag.
- `src/coding/modes/proxy.zig` — same flag pattern; closes the listen socket
  (`server.deinit(io)`) before calling `spawnAndExit`. `POST /restart` route
  handler and the RPC bridge both set the flag.
- `src/agent/guardrails/finish_task.zig` — `FinishTaskParams` gains a
  `restart: bool = false` field.
- `src/agent/guardrails/guardrails.zig` — `GuardrailState` gains
  `restart_requested: ?*std.atomic.Value(bool)` wired by the mode driver.

Design:
[`docs/design/decided/v2.17-self-restart.md`](../../docs/design/decided/v2.17-self-restart.md).
